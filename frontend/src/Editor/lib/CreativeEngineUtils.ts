...
export const zoomToSelectedText = async (
    ...
    const overlapBottom = Math.max(
        (canvasBounding.top +
            ...
            (window.visualViewport?.height ?? canvasBounding.height)) *
...
export const pixelToCanvasUnit = (engine : any, pixel : any) => {
...
export const autoPlaceBlockOnPage = (
    ...
    engine.block
        .findAllSelected()
        .forEach((blockId : any) => engine.block.setSelected(blockId, false));
...
export function getImageSize(url : any) {
