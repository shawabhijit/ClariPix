...
export async function initPhotoEditorUIConfig(
    ...
    try {
        ...
        console.error('Error initializing photo editor UI config:', error);
        throw error;
    }
}
...
function setupInspectorBar(instance: CreativeEditorSDK) {
    ...
    const unsubscribe = instance.engine.block.onSelectionChanged(() => {
        ...
        const isPage = selectedBlock && instance.engine.block
...
function setupDock(instance: CreativeEditorSDK) {
    ...
    // Additional dock components setup...
    instance.ui.registerComponent(
...
async function setupPhotoEditingScene(
    ...
    try {
        const size = await getImageSize(uri) as { width: number; height: number } | null;
        ...
        // Create scene only if no scene exists
        let scene = engine.scene.get();
        if (!scene) {
            scene = engine.scene.create('Free');
        }
        ...
        // Disable stroke of page, since it does not make sense with current wording and takes up too much space
        ...
        const unsubscribeSelectionChange = engine.block.onSelectionChanged(() => {
            ...
            if (selection.length === 0) {
                ...
                if (currentPage) {
                    engine.block.select(currentPage);
                }
            }
        });
        ...
        console.log('Photo editing scene setup completed successfully');

        return () => {
            unsubscribeSelectionChange();
        };
    } catch (error) {
        console.error('Error setting up photo editing scene:', error);
        throw error;
    }
}
