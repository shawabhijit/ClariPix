...
export const ResultPage = () => {
    ...
    const bgChanged = appContext?.bgChanged;
    ...
    const handleBgChnageByPrompt = (selectedImage: any, prompt: string) => {
        setIsGenerating(true);
        generateByPrompt?.(selectedImage, prompt);
    }
    ...
    useEffect(() => {
        if (!resultImage && !image) {
            bgChanged ? navigate("/change-background") : navigate("/remove-bg")
        }
        if (bgChanged === false) {
            setIsGenerating(true);
        }
        ...
        <div className="min-h-screen relative bg-primary/15">
            <div className="container mx-auto px-4 py-8">
                ...
                <header className="animate-fade-in">
                    <Button
                        variant="outline"
                        onClick={() => bgChanged ? navigate("/change-background") : navigate("/remove-bg")}
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-4">
                                            <div className="flex-1">
                                                <Input
                                                    ...
                                                    onKeyDown={(e: any) => e.key === "Enter" && handleBgChnageByPrompt(image, prompt)}
                                                />
                                            </div>
                                            <Button
                                                onClick={() => handleBgChnageByPrompt(image, prompt)}
